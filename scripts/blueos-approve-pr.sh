...
unsubscribe_approval_user() {
  ...
  if ! jq -e '.errors == null and
    ...
    echo "Failed to unsubscribe approval user from $PR_URL" >&2
    jq -c '{errors,viewerSubscription:.data.updateSubscription.subscribable.viewerSubscription}' \
      <<< "$response" >&2
    exit 1
  fi
  echo "Unsubscribed approval user from $PR_URL"
}
